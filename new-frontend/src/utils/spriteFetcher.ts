...
export class SpriteFetcher {
  private availableCharacters: number[];
  private nodeCharacterMap: Map<string, number>;
  private unassignedCharacters: number[];
   ...
   * @param node_id - Node ID used to bind a character.
   * @param stance - Stance ('D', 'L', 'R', 'U').
   * @param frame - Frame number (1, 2, 3).
   * @returns Image path.
   */
  fetchSprite(node_id: string | null = null, stance: string = 'D', frame: number = 1): string {
   ...
   * @returns Usage status summary.
   */
  getStatus(): {
    totalCharacters: number;
    assignedNodes: number;
    unassignedCount: number;
    nodeCharacterMap: Record<string, number>;
